...
use mpp_mod,                  only: input_nml_file, mpp_sum, mpp_error, mpp_max, mpp_min, mpp_pe
...
! Stop freezing and prevent Salinity from going crazy
!logical :: limit_temp = .true.
!logical :: limit_salt = .true.
     ...
     if (Sponge(n)%id > 0) then
        ...
        call time_interp_external(Sponge(n)%id, Time%model_time, wrk1) 
           ...
           if ( first_pass ) then
                 ...
                 do k = 1,nk
                    do j=jsd,jed
                       ...
                       if ( use_increment ) then
                         T_prog(n)%field(i,j,k,taum1)=T_prog(n)%field(i,j,k,taum1)+wrk1(i,j,k)
                         T_prog(n)%field(i,j,k,tau)=T_prog(n)%field(i,j,k,tau)+wrk1(i,j,k)
                         else
              ...
              if ( use_hard_thump .and. use_increment ) then
              wrk1=T_prog(n)%field(:,:,:,taum1)
              endif
...
!                 wrk2(i,j,k) = wrk2(i,j,k)+Thickness%rho_dzt(i,j,k,tau)*min(42.5-T_prog(n)%field(i,j,k,taum1),0.0)/3600.0
!              enddo
!           enddo
!        enddo
!     endif
